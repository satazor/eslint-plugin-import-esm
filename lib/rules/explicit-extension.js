...
module.exports = {
  meta: {
    ...
    docs: {
      description:
        'Validate if import and export paths have an explicit extension',
      recommended: true
    },
    fixable: 'code',
    schema: [
  ...
  create(context) {
    ...
    const configuredExtensions = Array.isArray(options.extension)
      ...
      ? [options.extension]
      : null;
    const testExistingFiles = options._test_existing_files || [];
    ...
    const findExistingFile = (basePath) => {
      ...
      if (configuredExtensions) {
        if (testExistingFiles.length > 0) {
          // Test mode with configured extensions
          for (const ext of configuredExtensions) {
            if (testExistingFiles.includes(basePath + ext)) {
              return ext;
            }
          }
      ...
      try {
        ...
        const exactMatch = files.find(
          (file) => file === baseFileName + defaultExtension
        );
        if (exactMatch) {
        ...
        const matchingFile = files.find(
          (file) => file.startsWith(baseFileName + '.') && file !== baseFileName
        );
        ...
        fix: !canFix
          ...
          : (fixer) => {
              ...
              if (isDirectory(absoluteImportPath)) {
                ...
                const basePathWithoutExt = ext
                  ? absoluteImportPath.slice(0, -ext.length)
                  : absoluteImportPath;
                absoluteImportPath =
                  basePathWithoutExt + findExistingFile(basePathWithoutExt);
              } else if (ext) {
