...
module.exports = {
  ...
  create(context) {
    ...
    const keepAsIsExtensions = new Set([
      // Data files
      '.json',
      // Styles
      '.css',
    ...
    const handleImportOrExportNode = (node) => {
      ...
      const isPackageImport = !isRelativeImport && !importPath.startsWith('/');
      const ext = path.extname(importPath);
      const hasJsExtension = ext === defaultExtension;

      // Skip if:
      // - it's a package import (non-relative path)
      // - it already has .js extension
      // - it has a non-JS extension that should be kept as-is (like .json)
      if (isPackageImport || hasJsExtension || keepAsIsExtensions.has(ext)) {
        return;
      }
